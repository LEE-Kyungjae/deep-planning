...
from palamedes_agents.insight_persistence import insight_write_payload, persist_reference_insights
from palamedes_agents.reference_rag import build_reference_rag_context
from palamedes_agents.reference_store import ReferenceStore
from palamedes_agents.strategy_llm import StaticStrategyProvider, run_strategy_llm
...
class InsightPersistenceTests(unittest.TestCase):
    ...
    def test_persist_reference_insights_updates_core_state_idempotently(self):
        with PalamedesStateIsolation():
            adapter = PalamedesAdapter(build_client(ClientConfig(mode="in-process")), require_healthy_writes=False)
    ...
    def test_store_retrieval_strategy_and_core_persistence_e2e(self):
        with PalamedesStateIsolation() as isolated:
